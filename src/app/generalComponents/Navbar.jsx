...
/*   useEffect(() => {
  console.log("Session changed:", session, status);
    ...
    return (
    <div className='h-[80px] bg-base-100 shadow-sm p-4 sm:px-6 lg:px-8 flex items-center'>
<div className="navbar max-w-7xl w-full h-full mx-auto">
  ...
  <div className="navbar-end flex gap-6">
    <div className='flex gap-4'>
      {
        status === "authenticated" ?
        <button onClick={() => signOut()} className='btn bg-red-600 text-white rounded-lg'>Logout</button>
       :
      <Link href={'/login'}><button className='btn rounded-lg'>Login</button></Link>
      }
    </div>
{/*     <Link href={'/login'}>
